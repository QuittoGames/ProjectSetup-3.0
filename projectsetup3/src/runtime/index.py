...
from projectsetup3.src.app.Services.UI.ArrowsService import ArrrowsService
...
def draw_main_dashboard():
    ...
    # Esquerda - User
    left_text = Text.assemble(("● ", success_color), (f"{info['user']}", accent_color))
    ...
    # Painel taskbar
    taskbar_panel = Panel(
        taskbar, box=ROUNDED, border_style=theme_color, padding=(0, 2), width=90
    )
...
def make_simple_header(title_str="Menu"):
    ...
    header_panel = Panel(
        header_grid, box=ROUNDED, border_style=theme_color, padding=(0, 2), width=60
    )
    ...
    for file_path in sorted(base.keys()):
        ...
        if "/" in file_path:
            ...
            for folder in folders:
                ...
                accumulated_path = (
                    f"{accumulated_path}/{folder}" if accumulated_path else folder
                )

                if accumulated_path not in folder_map:
                    # Adiciona a pasta na árvore e guarda a referência
                    folder_map[accumulated_path] = current_branch.add(
                        f"[bold {text_main}]📂 {folder}[/]"
                    )
...
def save_user_config(user_config):
    ...
    colors_config = {
        ...
        if "color" in k or k in ["text_dim", "text_main"]
    }
    if colors_config:
...
def save_all_config():
    ...
    preview_table.add_row(
        "  Padrão:", Text(Config.BASECODEEDITOR.upper(), style=accent_color)
    )
    ...
    preview_table.add_row(
        "  Git:",
        Text(
            "✓ ATIVO" if Config.GitAvaliable else "✗ DESATIVADO",
    ...
    preview_table.add_row(
        "  README IA:",
        Text(
            "✓ ATIVO" if Config.READMEAvaliable else "✗ DESATIVADO",
    ...
    preview_table.add_row(
        "  Histórico:",
        Text(
            "✓ ATIVO" if Config.HistoryAvaliable else "✗ DESATIVADO",
    ...
    preview_table.add_row(
        "  Python:", Text(str(Config.DIRETORIO)[-30:], style=accent_color)
    )
    preview_table.add_row(
        "  Web:", Text(str(Config.DIRETORIO_WEB)[-30:], style=accent_color)
    )
    preview_table.add_row(
        "  C++:", Text(str(Config.DIRETORIO_CPP)[-30:], style=accent_color)
    )
    ...
    options = [f"  ✓ Sim, salvar agora   ", f"  ✗ Cancelar            "]
    ...
    if choice == 0:
        ...
        with console.status(
            f"[{theme_color}]💾 Salvando configurações...[/]", spinner="dots"
        ):
            try:
                # Salva usando o método do backend
                ...
                success_table.add_row(
                    Text("✓", style=f"bold {success_color} blink", justify="center")
                )
                success_table.add_row("")
                success_table.add_row(
                    Text("CONFIGURAÇÕES SALVAS!", style=f"bold {success_color}")
                )
                success_table.add_row("")
                success_table.add_row(
                    Text("Suas preferências foram armazenadas", style=text_dim)
                )
                success_table.add_row(
                    Text("e serão carregadas automaticamente", style=text_dim)
                )
                success_table.add_row("")
                success_table.add_row(Text("📁 Localização:", style=text_dim))
                success_table.add_row(
                    Text(
                        str(config.appdata / "Config" / "config.json"),
                        style=accent_color,
                    )
                )
                ...
                error_text.append(
                    "Tente novamente ou verifique as permissões", style=text_dim
                )
...
def config_screen():
    ...
    while True:
        ...
        config_display.add_row(
            "Git:", "✓ Ativo" if Config.GitAvaliable else "✗ Desativado"
        )
        config_display.add_row(
            "README IA:", "✓ Ativo" if Config.READMEAvaliable else "✗ Desativado"
        )
        config_display.add_row(
            "Histórico:", "✓ Ativo" if Config.HistoryAvaliable else "✗ Desativado"
        )
        config_display.add_row("", "")
        config_display.add_row(
            "Dir. Python:",
            str(Config.DIRETORIO)[:50] + "..."
            if len(str(Config.DIRETORIO)) > 50
            else str(Config.DIRETORIO),
        )
        config_display.add_row(
            "Dir. Web:",
            str(Config.DIRETORIO_WEB)[:50] + "..."
            if len(str(Config.DIRETORIO_WEB)) > 50
            else str(Config.DIRETORIO_WEB),
        )
        config_display.add_row(
            "Dir. C++:",
            str(Config.DIRETORIO_CPP)[:50] + "..."
            if len(str(Config.DIRETORIO_CPP)) > 50
            else str(Config.DIRETORIO_CPP),
        )
        config_display.add_row("", "")
...
def edit_editor_config():
    ...
    editor_text = Text()
    editor_text.append(
        "Selecione seu editor preferido:\n\n", style=f"bold {theme_color}"
    )
    ...
    if choice < len(editors):
        ...
        console.print(
            Align.center(
                f"[{success_color}]✓ Editor alterado para: {selected_editor}[/]"
            )
        )
        time.sleep(1.5)
...
def toggle_git_config():
    ...
    while True:
        ...
        if Config.GitAvaliable:
            ...
            status_text.append(
                "  • Inicialização automática de repositório\n", style=accent_color
            )
            ...
            status_text.append(
                "Você pode inicializar manualmente depois\n\n", style=text_dim
            )
        ...
        if Config.GitAvaliable:
            ...
            status_text.append(
                "  ✓ git commit -m 'Initial commit'", style=success_color
            )
        else:
...
def toggle_readme_config():
    ...
    while True:
        ...
        if Config.READMEAvaliable:
            ...
            status_text.append(
                "  • Análise de descrição do projeto\n", style=accent_color
            )
            status_text.append("  • Seções personalizadas\n", style=accent_color)
            status_text.append(
                "  • Badges e formatação Markdown\n\n", style=accent_color
            )
            status_text.append("🔋 Powered by: Gemini 2.5 Flash\n\n", style=text_dim)
        else:
            status_text.append("✗ DESATIVADO\n\n", style=warning_color)
            status_text.append(
                "README.md não será gerado automaticamente\n", style=text_dim
            )
...
def view_history():
    ...
    if len(projects) > 15:
        info_text = Text()
        info_text.append(
            f"Mostrando últimos 15 de {len(projects)} projetos\n", style=text_dim
        )
        info_text.append(
            f"Arquivo: {Config.baseDiretoryHistory / 'history.json'}", style=text_dim
        )
...
def toggle_history_config():
    ...
    while True:
        ...
        if Config.HistoryAvaliable:
            ...
            status_text.append("Recursos habilitados:\n", style=text_main)
            status_text.append(
                "  • Registro automático de projetos\n", style=accent_color
            )
            ...
            status_text.append(
                f"📁 Localização: {Config.baseDiretoryHistory}\n\n", style=text_dim
            )
        else:
            status_text.append("✗ DESATIVADO\n\n", style=warning_color)
...
def edit_single_directory(config_key, display_name):
    ...
    dir_text = Text()
    dir_text.append(
        f"Editar diretório padrão para {display_name}\n\n", style=f"bold {theme_color}"
    )
    ...
    if new_path:
        ...
        console.print()
        console.print(
            Align.center(f"[{success_color}]✓ Diretório {display_name} atualizado[/]")
        )
        time.sleep(1.5)
...
def edit_colors_config():
    ...
    while True:
        ...
        color_display.add_row(
            "Tema Principal:", Text(f"● {theme_color}", style=theme_color)
        )
        color_display.add_row(
            "Destaque:", Text(f"● {accent_color}", style=accent_color)
        )
        color_display.add_row(
            "Sucesso:", Text(f"● {success_color}", style=success_color)
        )
        ...
        color_display.add_row(
            "Texto Principal:", Text(f"● {text_main}", style=text_main)
        )
...
def restore_default_config():
    ...
    options = ["  Sim, restaurar       ", "  Não, cancelar        "]
...
def create_project_interactive():
    ...
    console.print(
        Align.center(
            f"[{text_dim}]Linguagem (python/web/lua/fastapi) [padrão: python][/]"
        )
    )
    console.print()
    ...
    if config.GitAvaliable:
        ...
        if use_git in ["y", "yes", ""]:
            ...
            console.print(
                Align.center(f"[dim](Ex: https://github.com/user/repo.git)[/]")
            )
            console.print()
            ...
            if gitRepoLink:
                console.print()
                console.print(
                    Align.center(f"[{success_color}]✓ Repositório configurado[/]")
                )
                time.sleep(1)
    ...
    if config.READMEAvaliable:
        ...
        readme_intro.append(
            "Descreva o objetivo do seu projeto em detalhes.\n", style=text_dim
        )
        readme_intro.append(
            "Isso será usado pela IA para gerar um README.md profissional.\n\n",
            style=text_dim,
        )
        readme_intro.append("💡 Dicas:\n", style=accent_color)
        readme_intro.append(
            "• Múltiplas linhas: pressione Enter para nova linha\n", style=text_dim
        )
        readme_intro.append(
            "• Finalize: deixe uma linha em branco e pressione Enter\n", style=text_dim
        )
        readme_intro.append(
            "• Pular: pressione Enter diretamente na primeira linha\n\n", style=text_dim
        )
        readme_intro.append("Exemplo:\n", style=accent_color)
        readme_intro.append(
            "'Sistema de gerenciamento de projetos com CLI interativa.\n",
            style=text_dim,
        )
        readme_intro.append(
            "Suporta Python, JavaScript e outras linguagens.\n", style=text_dim
        )
        readme_intro.append(
            "Inclui geração automática de README com IA.'", style=text_dim
        )
        ...
        console.print(
            Align.center(
                f"[{accent_color}]Digite sua descrição (linha vazia para finalizar):[/]"
            )
        )
        console.print()
        ...
        if projectDescription:
            ...
            preview_text = (
                ...
                if len(projectDescription) > 100
                else projectDescription
            )

            success_readme_text = Text()
            success_readme_text.append(
                "✓ Descrição capturada!\n\n", style=success_color
            )
            success_readme_text.append("Preview:\n", style=text_dim)
            success_readme_text.append(f'"{preview_text}"\n\n', style=accent_color)
            success_readme_text.append(
                f"Total: {len(projectDescription)} caracteres, {len(description_lines)} linhas",
                style=text_dim,
            )
            ...
            skip_readme_text = Text.assemble(
                ("✘ ", warning_color), ("README.md não será gerado", text_dim)
            )
    ...
    try:
        ...
        dir_panel = Panel(
            Align.center(
                Text.assemble(
                    ...
                    (
                        f"{gitRepoLink}\n" if gitRepoLink else "",
                        accent_color if gitRepoLink else "",
                    ),
                    ("README IA: ", text_dim),
                    (
                        f"{'Ativo' if projectDescription else 'Desativado'}",
                        success_color if projectDescription else warning_color,
                    ),
                )
            ),
        ...
        try:
            ...
            console.print(
                Align.center(
                    f"[{warning_color}]⚠️ Aviso: Falha ao carregar estrutura base ({e}), usando fallback 'TEXT'[/]"
                )
            )
            time.sleep(1)
...
async def Start():
    while True:
        ...
        console.print(
            Align.center(
                Text(
                    "Use ↑↓ ou j/k para navegar | Enter para selecionar", style=text_dim
                )
            )
        )
        console.print()
