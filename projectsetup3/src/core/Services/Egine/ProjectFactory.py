...
from projectsetup3.src.models.Project import Project
from projectsetup3.src.config.Config import Config
from projectsetup3.src.core.tool import tool
from projectsetup3.src.models.Enums.RegistredProjectType import (
    RegistredProjectType as ProjectType,
)
from projectsetup3.src.Services.AI.READMEService import READMEService
from projectsetup3.src.Services.core.HistoryService import HistoryService
import re
import os
import json
import datetime
...
class ProjectFactory:
    def create(
        ...
        if project_raw.basestruture is None:
            try:
                project: Project = project_raw.openBaseCodeJson(project_raw)
            except Exception as e:
                raise RuntimeError(
                    f"Base structure not available and couldn't be loaded: {e}"
                )
        ...
        for file, code in project.basestruture.items():
            ...
            if Config.READMEAvaliable and content and file == "README.md":
                code = READMEService.genereteREADME(
                    content, name, project.language.value, project.basestruture
                )
    ...
    def loadProjectConfiguration(self, project: Project) -> Project | None:
        ...
        if not os.path.isfile(projectPath):
            raise FileNotFoundError(
                f"Json file for {self.language.name} "
                f"(tried: {self.language.name.lower()}.json) not found in {Config.basesCodesPath}"
            )
