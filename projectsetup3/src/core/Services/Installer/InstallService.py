...
class InstallService:
    ...
    def install(config: Config) -> None:
        ...
        if not appdata_local.exists() or not all(
            (appdata_local / f).exists() for f in ["python.json", "java.json"]
        ):
    ...
    def checkDependencies():
        ...
        for dep in deps:
            if shutil.which(dep) is None:
                console.print(
                    f"[bold yellow]Aviso:[/bold yellow] {dep} não encontrado."
                )
                ...
                if resposta == "y":
                    if platform.system() == "Windows":
                        try:
                            subprocess.run(
                                f"winget install --id {dep} -e", shell=True, check=True
                            )
                        except subprocess.CalledProcessError:
                            console.print(
                                f"[bold red]Erro:[/bold red] Não foi possível instalar {dep} automaticamente."
                            )
                            return
                    return
                # Linux
                try:
                    ...
                    if cmd:
                        ...
                        print(
                            f"Gerenciador {cmd} detectado, mas não há comando definido. Instale {dep} manualmente."
                        )
    ...
    def getData():
        ...
        try:
            subprocess.run(command, shell=True, check=True)
            console.print(
                "[bold green]✓[/bold green] Arquivos de linguagem baixados com sucesso!"
            )
        except subprocess.CalledProcessError as e:
